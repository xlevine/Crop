...
import cartopy
from dic_modis import data_folder, find_dataset_varname
from read_hdf_file import read_var_hdf
...
def read_modis_field(varname,timestamp,fips):

    dataset_name = find_dataset_varname(varname,timestamp)
    [datafolder, time_format, nom_res] = data_folder(dataset_name,timestamp)

    [locator_MODIS, extent_county] = find_tiles_and_pixels(fips,nom_res)
        ...
        for V in np.arange(V_min,V_max+1,1):    
            ...
            for H in np.arange(H_min,H_max+1,1):
                ...
                try:
                    ...
                    pixels_tile = []
                    [lon_tile_2D_T, lat_tile_2D_T] = define_SIN_axis(region,nom_res)
...
def find_tiles_and_pixels(fips,nom_res):

    if str(fips).isnumeric() is True:
        [county_shape, county_coord, county_fips, county_name] = find_county_coordinates(fips)
    else:
        [county_shape, county_coord, county_fips, county_name] = find_state_coordinates(fips)
    ...
    [locator_MODIS, extent_county] = find_SIN_indices(lon_points,lat_points,lon_points_in_holes,lat_points_in_holes,county_shape,nom_res)

    return locator_MODIS, extent_county

def find_SIN_indices(lon_points_,lat_points_,lon_points_in_holes,lat_points_in_holes,county_shape,nom_res):
    ...
    if county_shape=='MultiPolygon':
        ...
        for m in np.arange(0,np.shape(lon_points_)[0],1):
            ...
            poly = Polygon(coords)
            [locator_MODIS_T, extent_county_T] = find_pixels_in_poly(lon_points,lat_points,poly,nom_res)
        ...
        [locator_MODIS_T, extent_county] = find_pixels_in_poly(lon_points,lat_points,poly,nom_res)
        H_poly_.extend(locator_MODIS_T['H'])
        V_poly_.extend(locator_MODIS_T['V'])
        pixels_poly_.extend(locator_MODIS_T['pixels'])
        lon_poly_.extend(locator_MODIS_T['lon'])
...
def find_pixels_in_poly(lon_points,lat_points,poly,nom_res):
    ...
    [H1,V1] = find_SIN_tiles(lat_max,lon_min,nom_res)
    [H2,V2] = find_SIN_tiles(lat_max,lon_max,nom_res)
    [H3,V3] = find_SIN_tiles(lat_min,lon_min,nom_res)
    [H4,V4] = find_SIN_tiles(lat_min,lon_max,nom_res)
    ...
    for tile in tiles:
        ...
        region = 'h'+ str('{0:02d}'.format(H)) + 'v'+ str('{0:02d}'.format(V))
        [lon_2d, lat_2d] = define_SIN_axis(region,nom_res)
...
def find_state_coordinates(state_abbrv):
    ...
    state_name = state_infos['properties']['name']

    state_id = int(''.join([n for n in state_strid if n.isdigit()]))

    return state_shape, state_coord, state_id, state_name

    [state_shape, state_coord, state_id, state_name] = find_state_coordinates(fips_str)
...
def find_county_coordinates(fips):
    ...
    return county_shape, county_coord, county_fips, county_name

# my code only deal with SIN grids
def find_SIN_tiles(lat,lon,nom_res):
...
def define_SIN_axis(region,nom_res):
...
################# PLOTTING #################
def map_landuse_2D(year_str,fips_str):
    ...
    try:
        if int(fips_str[0])==0:
            ...
            fips = int(fips_str) 
        [county_shape, county_coord, county_fips, county_name] = find_county_coordinates(fips)
        location_name = county_name + ' county '
    except (ValueError):
        fips = fips_str
        [county_shape, county_coord, county_fips, county_name] = find_state_coordinates(fips)
        location_name = county_name

    [data_county_2D, lon_county_2D, lat_county_2D, extent_county] = read_modis_field(varname,year_str,fips)
...
#    PLOT
    ...
    plt.title(location_name + ' in ' + year_str)
    ax.add_feature(rivers_50m, facecolor='None', edgecolor='b')
    save_file = fips_str + '.png'
